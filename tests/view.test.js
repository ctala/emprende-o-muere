...
test('labor rows show honest yield ranges, never a single fake number', () => {
  ...
  assert.equal(build.desc, 'Contactos +5–11 · Moral −8', 'tier alto: rango 8±3, no un +8 suelto');
  assert.match(healthy.rows.find((r) => r.id === 'TALK_TO_CUSTOMERS').desc, /^Contactos \+3–9/);
  assert.match(healthy.rows.find((r) => r.id === 'PUBLISH_CONTENT').desc, /^Contactos \+1–7/);

  const mid = model({ ...createGame(1), teamMorale: 50 });
  assert.equal(mid.rows.find((r) => r.id === 'BUILD_PRODUCT').desc, 'Contactos +2–5 · Moral −8 · moral baja',
...
test('hire row sells the perk, not just the cost', () => {
  ...
  assert.match(hire.desc, /Gastos suben a 17k\/mes/);
  ...
  assert.match(hire.desc, /\+6 contactos\/mes/, 'VENTAS pipeline must be visible');
...
test('every visible row answers "what do I get / what does it cost": honest descs', () => {
  ...
  assert.match(close.desc, /Contactos −10/, 'cost shown');
  ...
  assert.match(pitch.desc, /Valor actual de la empresa: \$198k/, 'pitch shows the live valuation anchor');
});

test('fresh state: hero matches floor(cash/burn), six rows, core-loop first', () => {
  const m = model(createGame(20260918));
  assert.equal(m.hero.value, `${Math.floor(120 / 15)} meses`);
  assert.equal(m.hero.gloss, 'si no entra plata nueva');
...
test('burned team (tier 0 morale): labor rows carry the burned warning', () => {
  ...
  for (const id of ['BUILD_PRODUCT', 'TALK_TO_CUSTOMERS', 'PUBLISH_CONTENT']) {
    ...
    assert.match(row.warn, /agotado/i, `${id} must warn about zero traction`);
  }
...
test('traction field speaks plain Spanish, not jargon', () => {
  const m = model(createGame(2));
  assert.equal(m.fields.find((f) => f.id === 'traction').label, 'Contactos');
});
...
test('fresh log shows a ledger line, never an empty band', () => {
  const m = model(createGame(7), { log: [] });
  assert.equal(m.log.length, 0);
  assert.match(m.logEmpty, /libro está limpio/);
});

test('no team: burn shows the total once, no parts echo (panel: redundante)', () => {
  const m = model(createGame(3));
  const burn = m.fields.find((f) => f.id === 'burn');
  assert.equal(burn.parts, null, 'parts appear only when salaries add parts');
  const m2 = model(createGame(3), { burn: 15 });
  assert.equal(m2.fields.find((f) => f.id === 'burn').parts, null);
});

test('energy carries its plain gloss inline (panel asked twice)', () => {
  const m = model(createGame(3));
  const energy = m.fields.find((f) => f.id === 'energy');
  assert.match(energy.gloss, /inversor/i);
  assert.match(energy.gloss, /Descansar/i);
});
